...
class NoReturnCVHittingTimeDistribution(AbstractCVHittingTimeDistribution, AbstractNoReturnHittingTimeDistribution):
    ...
    def _pdf(self, t):
        ...
        """
        # gauss = norm.pdf((self.x_predTo - self._ev_t(t))/np.sqrt(self._var_t(t)))  # Std. Gauss pdf
        # if t.ndim == 2:
        #     der_arg = self.x_L[:, np.newaxis, 1] / np.sqrt(self._var_t(t)) + (self.x_predTo - self._ev_t(t)) * (
        ...
        # else:
        #     der_arg = self.x_L[:, 1] / np.sqrt(self._var_t(t)) + (self.x_predTo - self._ev_t(t)) * (
...
class UniformCVHittingTimeDistribution(AbstractCVHittingTimeDistribution, AbstractUniformHittingTimeDistribution):
    ...
    def __init__(self, x_L, x_predTo, t_L, point_predictor, window_length, a=0.5, name='Uniform model'):
        ...
        # pos_last = x_L[:, [0, 2]]
        # v_last = x_L[:, [1, 3]]
        # t_predicted = point_predictor(pos_last, v_last)
